...
export default async function DashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-[#1C1C1E] px-5 py-6">
      {/* Header */}
      ...
      <section className="mb-8">
        ...
        {groups.length === 0 ? (
          <div className="bg-[#2C2C2E] rounded-2xl p-6 text-center">
            <p className="text-[#8E8E93] mb-4">Je bent nog geen lid van een groep</p>
            <Link href="/dashboard/groups/new">
              <button className="bg-[#3A3A3C] hover:bg-[#48484A] text-white font-medium rounded-xl px-6 py-2.5 text-sm transition-colors">
          ...
          <div className="space-y-3">
            {groups.map(({ group, role }) => (
              <Link key={group.id} href={`/dashboard/groups/${group.slug}`}>
                <div className="bg-[#2C2C2E] rounded-2xl p-4 hover:bg-[#3A3A3C] transition-colors cursor-pointer">
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-full bg-[#34C759] flex items-center justify-center text-white font-bold shrink-0">
        ...
        {recentJobs.length === 0 ? (
          <div className="bg-[#2C2C2E] rounded-2xl p-6 text-center">
            ...
            <Link href="/dashboard/jobs/new">
              <button className="bg-[#34C759] hover:bg-[#2DB84E] text-white font-medium rounded-xl px-6 py-2.5 text-sm transition-colors">
                ...
                <Link key={job.id} href={`/dashboard/jobs/${job.id}`}>
                  <div className="bg-[#2C2C2E] rounded-2xl p-4 hover:bg-[#3A3A3C] transition-colors cursor-pointer">
                    ...
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs">
                      ...
                      <span className="text-[#8E8E93]">{job.locationType}</span>
                      {job.hoursPerWeek && (
                        <span className="text-[#8E8E93]">{job.hoursPerWeek}u/w</span>
                      )}
                      {job.shares.length > 0 && (
